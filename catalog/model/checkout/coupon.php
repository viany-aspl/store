...
class ModelCheckoutCoupon extends Model {
	public function getCoupon($code) {
		 $log=new Log("order-coupon".date('Y-m-d').".log");
                             $log->write('get coupon called in model');
		$status = true;
		$log->write($this->session->data['coupon_store']);
		$coupon_store_id=$this->session->data['coupon_store'];
		...
		if ($coupon_query->num_rows) {
			...
			$coupon_product_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "coupon_product` WHERE coupon_id = '" . (int)$coupon_query->row['coupon_id'] . "' and store_id='".$coupon_store_id."' ");
			...
			$coupon_category_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "coupon_category` cc LEFT JOIN `" . DB_PREFIX . "category_path` cp ON (cc.category_id = cp.path_id) WHERE cc.coupon_id = '" . (int)$coupon_query->row['coupon_id'] . "' and cc.store_id='".$coupon_store_id."'");
