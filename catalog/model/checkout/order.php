...
class ModelCheckoutOrder extends Model {
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false) {
		$log=new Log("addOrderHistory-open-".date('Y-m-d').".log");
		$log->write('addOrderHistory Call');
try{
...
public function getgtax($order_id)
{
    ...
    $gtax_query = $this->db->query("select", "oc_order_total","","","", array('order_id'=>(int)$order_id , 'code'=>'tax'),'','',array('order_id','code','title','value','sort_order'));
    $log->write($gtax_query);
    
    $rows=$gtax_query->rows;
...
{
			$log=new Log("subsidy-change-".date('Y-m-d').".log");
                
                                          $gtax_query = $this->db->query("SELECT shipping_firstname,order_id FROM `oc_order` WHERE shipping_firstname <>'' and shipping_firstname <>'BARIPUR' and  shipping_firstname <>'PUSAPUR - 7701' and  shipping_firstname NOT REGEXP '^-?[0-9]+$'");
			//$log->write($gtax_query);			
				$gtax = $gtax_query->rows;
						//$log->write($gtax);
	return $gtax;
}
public function setsub($sname,$oid)
{
			$log=new Log("subsidy-set-change-".date('Y-m-d').".log");
                $sql="UPDATE `oc_order` set shipping_firstname=".$sname."  WHERE order_id=".$oid;
			$log->write($sql);
                                          $gtax_query = $this->db->query($sql);
