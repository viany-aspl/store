...
public function index() {
		...
		$results = $this->model_sale_order->getTop_5_Orders($filter_data);

		foreach ($results as $result) { 
		//print_r($this->currency->format($result['total'], 'INR', '1.00'));
			$data['orders'][] = array(
				'store_name'   => $result['store_name'],				
				'total'      => $this->currency->format($result['total'], 'INR', '1.00')
			);
		}
