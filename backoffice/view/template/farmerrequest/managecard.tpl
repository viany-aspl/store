...
return false;
    ...
    //alert(grower_id);
     $.ajax({
		url: 'index.php?route=farmerrequest/farmerrequest/check&token=<?php echo $token; ?>&grower_id=' +  encodeURIComponent(grower_id)+"&mobile="+encodeURIComponent(mobile)+'&unitid='+encodeURIComponent("2")+'&village_id='+encodeURIComponent(village_id),
		dataType: 'json',
			beforeSend:function()
			{
			$("#button-filter").hide();
		...
		success: function(json) {
               ...
               //Status Div check
                ...
                if(json.CARD_STATUS_DESC=="CARD REQUEST REJECTED")
                {
                    document.getElementById("issbtn").style.display = "block";
                    document.getElementById("cancelbtn").style.display = "block";  
                }
                //Status Div End
...
//                    }
                  ...
                  {
                     // alert('Ten');
                      document.getElementById("cardreqst").disabled = false;
                  }
                  else
                  {
                     // alert('Not Ten');
                      document.getElementById("cardreqst").disabled = true;
                  }
