...
<div id="content">
    ...
    <?php } ?>
      <!--<div class="pull-right">
          <a href="<?php echo $cancel; ?>" data-toggle="tooltip" title="<?php echo "Cancel Button"; ?>" class="btn btn-default">
		...
		<table class="table table-bordered">
		  ...
		  ?>
			...
			<?php	
			  ...
			  ?></td>
			  <td class="text-left remaining_quantity" style="width: 20%;"><?php if($product['received_products'] == 0) echo ''; else echo $product['quantity'] - $product['received_products'];?><span id ="remaining_quantity"><?php if(!isset($receive_bit)){ if(isset($product['rq'])) echo $product['rq']; ?></span> 
			  
			  <span>
			  <!-- | <button onclick="skipQuantity(this)" class="btn btn-primary" type="button">Skip Quantity</button></span>-->
			  <input type="hidden" class="rq" name="remaining_quantity[]" value="<?php if(isset($product['rq'])) echo $product['rq']; ?>"><?php } ?></td>
...
<?php //echo "here";print_r($order_information); ?>  
			...
			<td class="text-right" colspan="5" style="width: 100%;"><span class="input-group-btn">
                          ...
                          <button <?php if($order_information['order_info']['order_sup_send']!='0000-00-00'){ ?> disabled="disabled" <?php } ?> type="button" id="button-filter" class="btn btn-primary pull-right" onclick="submit_form()"><!--<i class="fa fa-search"></i>--> <?php echo "Confirm Order"; ?></button>
                        </span>
                        </td>
			</tr>
			<?php } ?>
