	...
	<div class="panel-body">
		...
		<table class="table table-bordered" id="print_table" border="1">
		  ...
		  ?>
            <tr id="tr_<?php echo  $a; ?>">
                ...
                <input class="form-control" name="p_amount[]" id="p_amount_<?php echo  $a; ?>" type="hidden" value="<?php echo (round($product['product_price']+$product['product_tax_rate'],PHP_ROUND_HALF_UP))*$product['product_quantity']; ?>" />
			  ...
			  <td class="text-left" id="td_p_price_<?php echo  $a; ?>">
                              <input required="required"  class="form-control"  onkeypress="return remove_zero(this.value,<?php echo $a; ?>);" onkeyup="return update_by_price(this.value,<?php echo $a; ?>);" name="p_price[]" id="p_price_<?php echo  $a; ?>" type="text" value="<?php echo round($product['product_price'],PHP_ROUND_HALF_UP);?>" />
                           </td>
			  ...
			  <td class="text-left" id="td_p_qnty_<?php echo  $a; ?>">
                              ...
                              <input  required="required" onkeypress="return remove_zero_q(this.value,<?php echo $a; ?>);" onkeyup="return update_by_q(this.value,<?php echo $a; ?>);" class="form-control" name="p_qnty[]" id="p_qnty_<?php echo  $a; ?>" type="text" value="<?php echo  $product['product_quantity'];?>" />
			  </td>
			  <td class="text-left" id="td_p_amount_<?php echo  $a; ?>">
			  <?php 
                          echo number_format((float)((round($product['product_price']+$product['product_tax_rate'],PHP_ROUND_HALF_UP))*$product['product_quantity']), 2, '.', '');
                          $grand_total=$grand_total+((round($product['product_price']+$product['product_tax_rate'],PHP_ROUND_HALF_UP))*$product['product_quantity']);
                          ?>
		...
		?>
            ...
            <table class="table table-bordered" id="print_table_2" border="1">
				...
				<td style="width: 22%;" class ="text-left" id="td_grand_total"><?php if($grand_total == 0) echo ''; else echo number_format((float)$grand_total, 2, '.', ''); ?></td>
			</tr>
...
function add_row(a)
{
    ...
    var html='<tr id="tr_'+count+'">'+
			  ...
			  '<td class="text-left" id="td_p_price_'+count+'">'+
                              '<input required="required" class="form-control" onkeypress="return remove_zero(this.value,'+count+');" onkeyup="return update_by_price(this.value,'+count+');" name="p_price[]" id="p_price_'+count+'" value="" type="text">'+
                           '</td>'+
			  ...
			  '<td class="text-left" id="td_p_qnty_'+count+'">'+
                              ...
                              '<input required="required" onkeypress="return remove_zero_q(this.value,'+count+');" onkeyup="return update_by_q(this.value,'+count+');" class="form-control" name="p_qnty[]" id="p_qnty_'+count+'" value="0" type="text">'+
			  '</td>'+
...
function remove_row(a)
{
    ...
    alertify.confirm('Are you sure ! you want to delete this product ?',
        ...
        var grand_total = parseFloat(gtotal) - parseFloat(row_amount);
...
function remove_zero_q(valuee,a)
{
    if(valuee==0)
    {
    $("#p_qnty_"+a).val('');
...
function update_by_q(valuee,a)
{
    ...
    //alert(p_price+'+'+tax_rate);
    var total_amount=(p_price+tax_rate)*(valuee);
    ...
    var grand_total=0;
        ...
        {
        ?>
        var p_amount=$('#p_amount_<?php echo $c; ?>').val();
        if(p_amount!=undefined)
        {
        var grand_total = parseFloat(grand_total) + parseFloat(p_amount);
        }
        <?php
        $c++; }
        ?>
...
function update_by_price(valuee,a)
{
    ...
    var tax_rate=parseFloat($("#p_tax_rate_"+a).val());
    var total_amount=((valuee)+(tax_rate))*($("#p_qnty_"+a).val());
    total_amount=parseFloat(total_amount).toFixed(2);
    $('#p_amount_'+a).val(total_amount);
    $('#td_p_amount_'+a).html(total_amount);
 ...
 <script type="text/javascript">
   ...
   $('input[name=\'product_name[]\']').autocomplete({
    ...
    'select': function(item) {
        ...
        /*
        $('#p_name_<?php echo $b; ?>').val(item['label']);
        $('#p_id_<?php echo $b; ?>').val(item['value']);
        $('#td_p_tax_type_<?php echo $b; ?>').html(item['product_tax_type']);
        $('#td_p_hsn_<?php echo $b; ?>').html(item['hstn']);
        $('#p_price_<?php echo $b; ?>').val(item['price_wo_t']);
        var total_amount=(parseFloat(item['price_wo_t'])+parseFloat(item['product_tax_rate']))*($("#p_qnty_<?php echo $b; ?>").val());
        total_amount=parseFloat(total_amount).toFixed(2);
        $('#p_amount_<?php echo $b; ?>').val(total_amount);
        $('#td_p_amount_<?php echo $b; ?>').html(total_amount);
        $('#p_tax_rate_<?php echo $b; ?>').val(item['product_tax_rate']);
        $('#td_p_tax_rate_<?php echo $b; ?>').html(item['product_tax_rate']);
        */
        ...
        var total_amount=(parseFloat(item['price_wo_t'])+parseFloat(item['product_tax_rate']))*($("#p_qnty_"+thisid).val());
        total_amount=parseFloat(total_amount).toFixed(2);
        ...
        var grand_total = parseFloat(grand_total) + parseFloat($('#p_amount_<?php echo $c; ?>').val());
        <?php
        $c++; }
        ?>
        grand_total=parseFloat(grand_total).toFixed(2);
