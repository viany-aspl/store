    ...
    <div class="panel panel-default">
	 ...
	 <td class="text-left">Submitted by</td>
	  ...
	  <td class="text-left"><?php echo $bill['submitted_by']; ?></td>
                  ...
                  <td class="text-left"><?php if($bill['payment_status']=="0"){ ?>
                      <a class="btn btn-primary pull-right" style="width: 61px;" href="index.php?route=tag/bills/accept&token=<?php echo $token; ?>&bill_id=<?php echo $bill["SID"]; ?>">Accept</a>
                      <br/>
                      <a class="btn btn-primary pull-right" onclick="return regQuery_bill_reject(<?php echo $bill["SID"]; ?>,'<?php echo $bill['submission_date']; ?>','<?php echo $bill['unit']; ?>','<?php echo $bill['store_name']; ?>','<?php echo $bill['date_start']; ?>','<?php echo $bill['date_end']; ?>','<?php echo $bill['amount']; ?>','<?php echo $bill["logged_user"]; ?>');" style="background-color: #D44F32;margin-top: 5px;width: 61px;" href="index.php?route=tag/billsubmission/reject&token=<?php echo $token; ?>&bill_id=<?php echo $bill["SID"]; ?>">Reject</a>
                      <?php
                       } ?></td>
                </tr>
                <?php } ?>
...
$("#input-store").select2();
function regQuery_bill_reject(bill_id,submission_date,unit,store_name,date_start,date_end,amount,logged_user)
{
    $('#bill_id').val(bill_id);
...
$('#button-filter').on('click', function() {
	url = 'index.php?route=tag/bills/getlist&token=<?php echo $token; ?>';
...
$('#button-download').on('click', function() {
	url = 'index.php?route=tag/bills/getlist_download&token=<?php echo $token; ?>';
