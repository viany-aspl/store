...
<div id="content">
    ...
    <?php } ?>
      ...
      <h1><?php echo "Stock Transfer Receive Order"; ?></h1>
      <ul class="breadcrumb">
		...
		<table class="table table-bordered">
          <thead>
            <tr>
			  ...
			  <td class="text-left" style="width:20%">Option Values</td>
		  ...
		  ?>
            <tr>
			  ...
			  <?php for($i = 0; $i<count($product['quantities']); $i++){?>
				<select style="display:none;" class="form-control" name="supplier[]" <?php if((!isset($validation_bit) && $product['quantities'][$i]!=0) || $disable_bit){?> disabled <?php } ?>>
			  ...
			  <input type="text" style ="float:left;width:50%;" <?php if($product['quantities'][$i] == 0){?> onblur="getRemainingQuantity(this)" <?php } else{?>onblur="getRQuantity(this)"<?php } ?> name="receive_quantity[]" value="<?php if($product['quantities'][$i] == 0){  echo '' . '"'; }elseif($product['prices'][$i] != 0){ echo $product['quantities'][$i];?>"  <?php }elseif($product['quantities'][$i] != 0){ echo $product['quantities'][$i] . '"'; } ?> placeholder="Receive Quantity" class="form-control receive_quantity" /><input type="text" style ="width:50%;" maxlength="1" name="price[]" value="<?php if($product['prices'][$i] == 0) echo '1' . '"'; else{ echo $product['prices'][$i];?>" disabled <?php } ?> placeholder="Price" class="form-control price" /><?php echo "<br />"; ?>
				<?php 
				}
				else
				{
		...
		?>
			...
			<tr>
				...
				<?php if(isset($ftime_bit)){
					?>
					<input type="text" name="order_receive_date" value="<?php if($order_information['order_info']['receive_date'] == '0000-00-00'){ echo ''.'"'; }else{ echo $order_information['order_info']['receive_date']; ?>" disabled<?php } ?> placeholder="Order Receive Date" data-date-format="YYYY-MM-DD" id="input-date-added" class="form-control" />
                <?php }else{ 
				?>
					<input type="text" name="order_receive_date" value="<?php if($order_information['order_info']['receive_date'] == '0000-00-00'){ echo ''.'"'; }else{ echo $order_information['order_info']['receive_date']; ?>"<?php } ?> <?php if(!isset($validation_bit)){?> disabled <?php }?> placeholder="Order Receive Date" data-date-format="YYYY-MM-DD" id="input-date-added" class="form-control" />
                <?php } ?>
                  <span class="input-group-btn">
                  <button type="button" class="btn btn-default"><i class="fa fa-calendar"></i></button>
                  </span></div></td>
			</tr>
			<?php /*$product['received_products'] == 0*/ if($order_information['order_info']['receive_date']=='0000-00-00'){?>
			<tr>
				<td class="text-right" colspan="5" style="width: 100%;"><span class="input-group-btn">
                    <button type="button" id="button-filter" class="btn btn-primary pull-right" onclick="submit_form()"><!--<i class="fa fa-search"></i>--> <?php echo "Receive Order"; ?></button>
            </span></td>
			</tr>
			<?php } ?>
          </tbody>
	...
	function getRemainingQuantity(evnt)
	{
		...
		if($(evnt).val())
		{
			...
			if(remaining_quantity > 0)
			{
				...
				var html = '';
					html += '<br />';
					html +='<select style="display:none;" class="form-control" name="supplier[]">';
				<?php foreach($suppliers as $supplier){ ?>
					html += '<option value="<?php echo $supplier['id']; ?>"><?php echo $supplier['first_name'] . " " . $supplier['last_name']; ?></option>';
				<?php } ?>
					html += '</select>';
					html += '<input type="text" style="float:left;width:50%" onblur="getRemainingQuantity(this)" name="receive_quantity[]" value="" placeholder="Receive Quantity" class="form-control receive_quantity" /><input type="text" style ="width:50%;" maxlength="1" name="price[]" value="1"  placeholder="Price" class="form-control price" />';
					$(evnt).parent().next().children('#remaining_quantity').text(remaining_quantity);
					$(evnt).parent().next().children('input.rq').val(remaining_quantity);
