...
<div id="content">
  ...
  <div class="container-fluid">
    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-coupon" name="form1" class="form-horizontal">
          ...
          <div class="tab-content">
            <div class="tab-pane active" id="tab-general">
			  ...
			  <div class="form-group">
                <label class="col-sm-2 control-label" for="input-store"><span data-toggle="tooltip" title="Search Store">Select Store</span></label>
                <div class="col-sm-10">
                  <input type="text" name="store" value="" placeholder="Stores" id="input-store" class="form-control" />
                  <div id="coupon-store" class="well well-sm" style="height: 150px; overflow: auto;">
                    <?php foreach ($coupon_store as $coupon_store) { ?>
                    <div id="coupon-store<?php echo $coupon_store['store_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $coupon_store['name']; ?>
                      <input type="hidden" name="coupon_store[]" value="<?php echo $coupon_store['store_id']; ?>" />
                    </div>
                    <?php } ?>
                  </div>
               
				<a style="cursor: pointer;" onclick="return select_all_stores();">Select All</a> 
				/ 
				<a style="cursor: pointer;" onclick="return unselect_all_stores();">Un Select All</a>
				 </div>
  ...
  /*
  alert("Submitted");
  var arr = document.form1['coupon_store[]'].value;
  alert(arr);
  return false;
  
    alert("Submitted");
	var selected_store=$('[name="coupon_store"]').val();//document.getElementsByName('coupon_store').value;
...
$('input[name=\'store\']').autocomplete({
	...
	'select': function(item) {
		...
		$('#coupon-store' + item['value']).remove();
		
		$('#coupon-store').append('<div id="coupon-store' + item['value'] + '"><i class="fa fa-minus-circle"></i> ' + item['label'] + '<input type="hidden" name="coupon_store[]" value="' + item['value'] + '" /></div>');	
	}
});

$('#coupon-store').delegate('.fa-minus-circle', 'click', function() {
	$(this).parent().remove();
});
...
{
$.ajax({
			url: 'index.php?route=marketing/coupon/getAllStores&token=<?php echo $token; ?>',
			//dataType: 'json',			
			success: function(json) {
				//alert(json);
				$('#coupon-store').html(json);
			}
		});
}
function unselect_all_stores()
{
//alert('kkkkkkk');
$('#coupon-store').html('');
return false;
