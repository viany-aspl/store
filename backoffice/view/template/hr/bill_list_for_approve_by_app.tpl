...
if($bill['users']!=$bill['employee_id'])
{
?>
		...
		<a onclick="return confirm('Are you sure you want to approve this bill ?');" class="btn btn-primary pull-right"  style="background-color: #9DD432;width: 71px;" href="index.php?route=hr/expenseapp/accept&token=<?php echo $token; ?>&bill_id=<?php echo $bill["tr_id"]; ?>">Approve  </a>
<br/>
...
<br/>
		<a class="btn btn-primary pull-right" onclick="return update_expense_reason('<?php echo $bill["tr_id"]; ?>','<?php echo $bill["logged_user"]; ?>');" style="margin-top: 5px;width: 71px;" href="#">EDIT</a>

	<?php } ?>
	<?php if($bill['status']=="1") { ?>
                <a class="btn btn-primary pull-right" onclick="return adjust_expense('<?php echo $bill["store_id"]; ?>','<?php echo $bill["amount"]; ?>','<?php echo $bill["expense_balance"]; ?>','<?php echo $bill["SID"]; ?>');" style="margin-top: 5px;width: 71px;" href="#">Adjust</a>
 <?php } ?>
...
<!-- Modal -->
  <div class="modal fade" id="myModal_partner2" role="dialog">
   ...
   <script type="text/javascript">
    function adjust_expense(store_id,amount,expense_balance,tr_id)
{
 // alert(amount+','+expense_balance);//return false;
  amount=parseFloat(amount);
  expense_balance=parseFloat(expense_balance);
    var url="index.php?route=hr/expenseapp/adjudteexpense&token=<?php echo $token; ?>&store_id="+store_id+"&amount="+amount+"&tr_id="+tr_id;
  //  alert(url);
        if(amount < expense_balance)
        {
        $.ajax({
  url: url,
  dataType: 'json',   
...
$('#button-filter').on('click', function() {
	url = 'index.php?route=hr/expenseapp&token=<?php echo $token; ?>';
...
$('#button-download').on('click', function() {
	url = 'index.php?route=hr/expenseapp/getlist_download&token=<?php echo $token; ?>';
