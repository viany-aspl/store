            ...
            <div class="col-sm-12">
               <div class="table-responsive">
                  ...
                  <?php } ?>
								...
								<div class="col-sm-6">
                              <div class="form-group">
									<label class="control-label" for="input-date-end">Store (Receiver)</label>
                               <select name="received_store" id="input-received_store" onchange="return get_prn_list(this.value);" class="form-control select2" style="width: 100%;">
									...
									<?php if ($error_received_store) { ?>
									<div class="text-danger"><?php echo $error_received_store; ?></div>
								<?php } ?>
                              </div>
                              </div>
									...
									<div class="col-sm-12">
                              <div class="form-group">
                             <label class="control-label" for="input-date-end">Purchase Request Number</label>
                              <select name="received_prn" id="input-received_prn" class="form-control select2">
										<option value=""> SELECT PURCHASE REQUEST NUMBER</option>
...
var received_store=$("#input-received_store").val();
...
if(input_date=="")
	...
	if(received_store=="")
    {
        alertify.error('Please Select Store(Receiver)');
        $("#input-received_store").focus();
        return false;
    }
	if(received_prn=="")
    {
        alertify.error('Please Select Purchase Request Number');
...
function get_prn_list(store_id)
{
	if(store_id=='')
	{
		$("#input-received_prn").html('<option value=""> SELECT PURCHASE REQUEST NUMBER</option>');
	}
	else
	{
		var product_id="<?php echo $order_information['order_info']['product_id'];  ?>";
		$.ajax({
		url:'index.php?route=purchaseorder/purchase_order/get_prn_list&token=<?php echo $token; ?>&store_id='+encodeURIComponent(store_id)+'&product_id=' +encodeURIComponent(product_id),
