...
class ModelInvoiceAdjustment extends Model {
        ...
        public function getInvoiceData($invoice_id)
	{
        ...
        public function getPartnerInfo($data) {
            $sql="select oc_store.*,(select oc_setting.value from oc_setting where oc_store.store_id=oc_setting.store_id and `key`='config_firmname') as firm_name from oc_store where store_id='".$data['filter_store']."'  ";
            $query = $this->db->query($sql);
            return $query->row;
