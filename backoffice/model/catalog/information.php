...
class ModelCatalogInformation extends Model {
	...
	public function getInformations($data = array()) {
		if ($data) {
                        /*
			$sql = "SELECT * FROM " . DB_PREFIX . "information i LEFT 
                         * JOIN " . DB_PREFIX . "information_description id ON 
                         * (i.information_id = id.information_id) 
			...
			return $query->rows;
                ...
                'localField' => 'information_id',
                'foreignField' => 'information_id',
                'as' => 'id'
            );
            $match=array('id.language_id'=>(int)$this->config->get('config_language_id'));
            if (isset($data['start']) || isset($data['limit'])) {
			if ($data['start'] < 0) {
				$data['start'] = 0;
			}
            ...
            $sort_array=array('id.title'=>1);
            $query = $this->db->query("join",DB_PREFIX . "information",$lookup,'$id',$match,'','',$limit,'',$start,$sort_array);
