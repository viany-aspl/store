...
class ModelMarketingAffiliate extends Model {
	public function addAffiliate($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "affiliate SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', salt = '" . $this->db->escape($salt = substr(md5(uniqid(rand(), true)), 0, 9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', company = '" . $this->db->escape($data['company']) . "', website = '" . $this->db->escape($data['website']) . "', address_1 = '" . $this->db->escape($data['address_1']) . "', address_2 = '" . $this->db->escape($data['address_2']) . "', city = '" . $this->db->escape($data['city']) . "', postcode = '" . $this->db->escape($data['postcode']) . "', country_id = '" . (int)$data['country_id'] . "', zone_id = '" . (int)$data['zone_id'] . "', code = '" . $this->db->escape($data['code']) . "', commission = '" . (float)$data['commission'] . "', tax = '" . $this->db->escape($data['tax']) . "', payment = '" . $this->db->escape($data['payment']) . "', cheque = '" . $this->db->escape($data['cheque']) . "', paypal = '" . $this->db->escape($data['paypal']) . "', bank_name = '" . $this->db->escape($data['bank_name']) . "', bank_branch_number = '" . $this->db->escape($data['bank_branch_number']) . "', bank_swift_code = '" . $this->db->escape($data['bank_swift_code']) . "', bank_account_name = '" . $this->db->escape($data['bank_account_name']) . "', bank_account_number = '" . $this->db->escape($data['bank_account_number']) . "',store_id='".$this->db->escape($data['user_store_id'])."',rabi_crop_1='".$this->db->escape($data['rabi_id1'])."',rabi_crop_2='".$this->db->escape($data['rabi_id2'])."',kharif_crop_1='".$this->db->escape($data['kharif_id1'])."',kharif_crop_2='".$this->db->escape($data['kharif_id2'])."',kharif_crop_1_acre='".$this->db->escape($data['kharif_acre1'])."',kharif_crop_2_acre='".$this->db->escape($data['kharif_acre2'])."',rabi_crop_1_acre='".$this->db->escape($data['rabi_acre1'])."',rabi_crop_2_acre='".$this->db->escape($data['rabi_acre2'])."' ,status = '" . (int)$data['status'] . "', date_added = NOW()");
	...
	public function getCrops() {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "crop");
	...
	public function getAffiliateByTelephone($telephone) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "affiliate WHERE LCASE(telephone) = '" . $this->db->escape(utf8_strtolower($telephone)) . "'");
...
$log=new Log("viewaff.log");
		...
		if (!empty($data['filter_store'])) {
			...
			$implode[] = "a.store_id = '" . $this->db->escape($data['filter_store']) . "'";
