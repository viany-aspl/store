...
class ModelReportProduct extends Model {
	...
	public function getTotalPurchased($data) {
		...
		*/
		$sql = "select count(*) as total,sum(total) as total_amount from (  SELECT op.name, op.model, SUM(op.quantity) AS quantity, SUM((op.total + op.tax) * op.quantity) AS total FROM " . DB_PREFIX . "order_product op LEFT JOIN `" . DB_PREFIX . "order` o ON (op.order_id = o.order_id)";
	 ...
	 public function getPurchasedCompanyWise($data = array()) {
		$sql = "SELECT op.name, op.model, SUM(op.quantity) AS quantity, 
		...
		return $query->rows;
        	...
        	public function getTotalPurchasedCompanyWise($data) {
	
		$sql = "select count(*) as total,sum(total) as total_amount from (  SELECT op.name, op.model, SUM(op.quantity) AS quantity, SUM((op.total + op.tax) * op.quantity) AS total FROM " . DB_PREFIX . "order_product op "
                        . "LEFT JOIN `" . DB_PREFIX . "order` o ON (op.order_id = o.order_id) "
                        . " left join oc_store as os on os.store_id = o.store_id";
