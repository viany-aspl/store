...
class ModelReportCustomerReport extends Model {
	...
	public function getOrders($data = array()) {
		/*$sql = "SELECT c.customer_id, "
                        ...
                        . " c.email, cgd.name AS customer_group, "
                        . "c.status, COUNT(o.order_id) AS orders,"
                ...
                */
                ...
                $sql = "select o.customer_id,"
                        ...
                        . "sum(p.quantity) as qnty, count(p.order_product_id) as dat "
                        . "from oc_order_product as p "
                        . "left join oc_order as o on p.order_id = o.order_id 
                         join oc_product_to_category on oc_product_to_category.product_id=p.product_id 
                         where  oc_product_to_category.category_id='1'
                        ";
		...
		if (!empty($data['filter_date_start'])) {
			$sql .= " AND DATE(p.ORD_DATE) >= '" . $this->db->escape($data['filter_date_start']) . "'";
		}

		if (!empty($data['filter_date_end'])) {
			$sql .= " AND DATE(p.ORD_DATE) <= '" . $this->db->escape($data['filter_date_end']) . "'";
		}

		$sql .= " group by o.telephone";
	...
	public function getTotalOrders($data = array()) {
		$sql = "select count(*) as total from (select o.customer_id,"
                        . "o.telephone as Mobile,"
                        . "count(p.product_id)as No_of_Product, "
                        . "sum(p.quantity)as qnty, date(p.ORD_DATE)as dat "
                        . "from oc_order_product as p "
                        . "left join oc_order as o on p.order_id = o.order_id 
			join oc_product_to_category on oc_product_to_category.product_id=p.product_id 
                         where  oc_product_to_category.category_id='1'
