...
class ControllerCommonFileManager extends Controller {
	...
	public function delete() 
	{
		...
		if (!$json) 
		{
			...
			try
			{
				$log=new Log("filemanager-cache-".date('Y-m-d').".log");
				$log->write($paths);
				$cache_array=array('-100x100.','-180x180.','-40x40.','-70x70.');
				foreach ($paths as $path) 
				{
					foreach($cache_array as $cac)
					{
						$path2=str_replace('.',$cac,$path);
						$log->write($path2);
						$path2 = rtrim(DIR_IMAGE .'cache/'. str_replace(array('../', '..\\', '..'), '', $path2), '/');
						$log->write($path2);
						// If path is just a file delete it
						if (is_file($path2)) 
						{
