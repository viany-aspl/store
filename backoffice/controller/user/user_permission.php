	...
	protected function getList() {
		...
		$filter_data = array(
			...
			'limit' => $this->config->get('config_limit_admin')
                ...
                $retdata=$this->model_user_user_group->getUserGroups($filter_data);

		$user_group_total = $retdata->num_rows;//$this->model_user_user_group->getTotalUserGroups();
		...
		foreach ($results as $result) {
			$data['user_groups'][] = array(
				...
				'edit'          => $this->url->link('user/user_permission/edit', 'token=' . $this->session->data['token'] . '&user_group_id=' . $result['user_group_id'] . $url, 'SSL'),
                                'menu_edit'       => $this->url->link('user/menu/groupmenu', 'token=' . $this->session->data['token'] . '&user_group_id=' . $result['user_group_id'] . $url.'&name='.$result['name'], 'SSL')
			);
		}
