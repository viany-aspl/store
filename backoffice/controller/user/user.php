...
class ControllerUserUser extends Controller {
	...
	protected function getList() 
	{
		...
		$data['user_groups'] = $this->model_user_user_group->getUserGroups()->rows;
        ...
        $retdata=$this->model_user_user->getUsers($filter_data);
		$user_total =$retdata[0]['totalrows'];// $this->model_user_user->getTotalUsers($filter_data);
		...
		foreach ($results as $result) {
			$data['users'][] = array(
				...
				'edit'       => $this->url->link('user/user/edit', 'token=' . $this->session->data['token'] . '&user_id=' . $result['user_id'] . $url, 'SSL'),
                                'menu_edit'       => $this->url->link('user/menu', 'token=' . $this->session->data['token'] . '&user_id=' . $result['user_id'] . $url.'&name='.$result['firstname']." ".$result['lastname'], 'SSL')
			);
		}
	...
	protected function getForm() 
	{
		...
		if (isset($this->error['State_Code'])) {
			$data['error_State_Code'] = $this->error['State_Code'];
		} else {
			$data['error_State_Code'] = '';
		}
		if (isset($this->error['Dist_Code'])) {
			$data['error_Dist_Code'] = $this->error['Dist_Code'];
		} else {
			$data['error_Dist_Code'] = '';
		}
		...
		if (isset($this->request->post['State_Code'])) {
			...
			$data['State_Code'] = $user_info['State_Code'];
		} else {
			$data['State_Code'] = '';
		}
		if (isset($this->request->post['State_Name'])) {
			...
			$data['State_Name'] = $user_info['State_Name'];
		} else {
			$data['State_Name'] = '';
		}
		if (isset($this->request->post['Dist_Code'])) {
			$data['Dist_Code'] = $this->request->post['Dist_Code'];
		} elseif (!empty($user_info)) {
			$data['Dist_Code'] = $user_info['Dist_Code'];
		} else {
			$data['Dist_Code'] = '';
		}
		if (isset($this->request->post['Dist_Name'])) {
			$data['Dist_Name'] = $this->request->post['Dist_Name'];
		} elseif (!empty($user_info)) {
			$data['Dist_Name'] = $user_info['Dist_Name'];
		} else {
			$data['Dist_Name'] = '';
		}
		...
		elseif (!empty($user_info)) 
		{
			 $data['image2'] = $user_info['images'][1];
		} 
		else 
		{
			$data['image2'] = '';
		...
		elseif (!empty($user_info) && $user_info['images'] && is_file(DIR_IMAGE .$user_info['images'][1])) 
		{
			$data['thumb2'] = $this->model_tool_image->resize($user_info['images'][1], 100, 100);
		} 
		else 
		{
		...
		$data['states'] = $this->model_sale_customer->getstates(array())->rows;
		if(!empty($data['State_Code'] ))
		{
			$data['districts'] = $this->model_sale_customer->getdisctricts(array('state_code'=>$data['State_Code'] ))->rows;
		}
		else
		{
	...
	{
        $this->load->model('sale/customer'); 
        if(isset($this->request->get['State_Code']))
        {
                $state_code=$this->request->get['State_Code'];
        ...
        {	
			$ret=$ret.'<option value="'.$ids['District_Code'].'">'.strtoupper($ids['District_Name']).'</option>';
		}
        echo $ret;
	...
	protected function validateForm() 
	{
		...
		if (empty($this->request->post['State_Code'])) 
		{
			$this->error['State_Code'] = 'Please Select State'; 
		}
		if (empty($this->request->post['Dist_Code']))
		{
			$this->error['Dist_Code'] = 'Please Select District'; 
		}
	...
	public function download_excel() 
	{
		...
		$this->load->model('user/user');
        $retdata=$this->model_user_user->getUsers($filter_data);
		$user_total =$retdata[0]['totalrows'];
        ...
        foreach ($results as $result)
        {
			...
			$date=date('m-d-Y',(strtotime($result['date_added'])));
			//exit;
            if($result['status']==1)
            { 
                $status='Enabled'; 
            ...
            $fdata=array(
                ...
                strtoupper($result['user_group_name']),
				$status,
                $date
                );
