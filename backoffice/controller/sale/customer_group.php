	...
	protected function getList() {
		...
		$filter_data = array(
			...
			'limit' => $this->config->get('config_limit_admin')
                ...
                $return_array=$this->model_sale_customer_group->getCustomerGroups($filter_data);
		$customer_group_total = $return_array->total_rows;//$this->model_sale_customer_group->getTotalCustomerGroups();

		$results = $return_array->rows;

		foreach ($results as $result) {
			$data['customer_groups'][] = array(
				'customer_group_id' => $result['customer_group_id'],
				'name'              => $result['cgd']['name'],
	...
	protected function getForm() {
		...
		if (isset($this->request->get['customer_group_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$customer_group_info = $this->model_sale_customer_group->getCustomerGroup($this->request->get['customer_group_id']);
                ...
                $customer_group_info=$customer_group_info[0];
