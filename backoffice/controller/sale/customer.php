...
class ControllerSaleCustomer extends Controller 
{
	...
	public function add() 
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
                        ...
                        $existing=$this->model_sale_customer->addCustomer($this->request->post);
                        if(empty($existing))
                        {
			...
			$this->response->redirect($this->url->link('sale/customer', 'token=' . $this->session->data['token'] . $url, 'SSL'));
                        ...
                        {
                            $this->session->data['error']='This Telephone number is already exist ! ';
                            //exit;
                        }
		}
...
{
		$filter_data = array(
			'filter_store_id'	=> $this->user->getStoreId(),		
			'filter_name'              => $filter_name,
			'filter_telephone'             => $filter_telephone,
			'filter_customer_group_id' => $filter_customer_group_id,
			'filter_status'            => $filter_status,
		...
		$query = $this->model_sale_customer->getCustomers($filter_data);
                
                $results=$query->rows;
                $customer_total = $query->num_rows;//$this->model_sale_customer->getTotalCustomers($filter_data);
		...
		$data['customer_groups'] = $this->model_sale_customer_group->getCustomerGroups();
                $data['customer_groups']=$data['customer_groups']->rows;
