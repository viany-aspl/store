...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() 
		...
		if (isset($this->request->get['billtype'])) {
			$billtype = $this->request->get['billtype'];
		} else {
			$billtype = null;
		}
...
$filter_data = array(
		...
		$query = $this->model_sale_order->getOrders($filter_data);
                $order_total = $query->num_rows;//$this->model_sale_order->getTotalOrders($filter_data);
                $results=$query->rows;    
		foreach ($results as $result) {
			$data['orders'][] = array(
				...
				'customer'      => $result['payment_firstname'].' ('.$result['telephone'].')',
				...
				'date_modified' => date($this->language->get('date_format_short'), (($result['date_modified']->sec))),
		...
		if (isset($this->request->get['billtype'])) {
			$url .= '&billtype=' . $this->request->get['billtype'];
		}
		...
		$data['billtype'] = (string)$billtype;
		$this->load->model('localisation/order_status');

		$data['order_statuses'] = $this->model_localisation_order_status->getOrderStatuses()->rows;
		...
		$data['payment_methods']=array('Cash','Cash Discount','Cash Credit','CC Discount','Credit','Credit Discount');
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['text_store_url'] = "Payment Method:";//$this->language->get('text_store_url');
			$data['text_customer'] = $this->language->get('text_customer');
			$data['text_customer_group'] = $this->language->get('text_customer_group');
			$data['text_email'] = "Telephone";//$this->language->get('text_email');
			$data['text_telephone'] = "Tagged:";//$this->language->get('text_telephone');
			$data['text_fax'] = "Cash:";//$this->language->get('text_fax');
			
			$data['text_tagged'] = "Credit:";//$this->language->get('text_telephone');
			$data['text_cash'] = "Cash:";//$this->language->get('text_fax');
			$data['text_subsidy'] = "Subsidy:";//$this->language->get('text_fax');
			...
			$data['store_url'] = $order_info['payment_method'];
			$data['firstname'] = $order_info['firstname'];
			$data['lastname'] = $order_info['lastname'];
			...
			$data['email'] = $order_info['telephone'];
			$data['telephone'] = $this->currency->format($order_info['tagged'],$order_info['currency_code'], $order_info['currency_value']);
			$data['tagged'] = $this->currency->format($order_info['tagged'],$order_info['currency_code'], $order_info['currency_value']);
			$data['fax'] = $this->currency->format($order_info['cash'],$order_info['currency_code'], $order_info['currency_value']);
			$data['cash'] = $this->currency->format($order_info['cash'],$order_info['currency_code'], $order_info['currency_value']);
			$data['subsidy'] = $this->currency->format($order_info['subsidy'],$order_info['currency_code'], $order_info['currency_value']);
			...
			$data['date_added'] = date($this->language->get('date_format_short'), ($order_info['date_added']->sec));
			$data['date_modified'] = date($this->language->get('date_format_short'), ($order_info['date_modified']->sec));
			$data['payment_firstname'] = $order_info['payment_firstname'];
			$data['payment_lastname'] = $order_info['payment_lastname'];
			$data['payment_company'] = $order_info['payment_company'];
			$data['payment_address_1'] = $order_info['payment_address_1'];
			...
			foreach ($products as $product) {
				...
				$options = array();//$this->model_sale_order->getOrderOptions($this->request->get['order_id'], $product['order_product_id']);
	...
	public function download_excel_item_wise() 
    {
            if (isset($this->request->get['filter_order_id'])) {
			$filter_order_id = $this->request->get['filter_order_id'];
		} else {
        ...
        foreach ($results as $result)
        {
            ...
            { 
                $billtype='Open Billing'; 
               
            } 
            else 
            { 
                $billtype='Inventory Led Billing';
                
            }
			foreach($result['order_product'] as $prd)
			{
				//print_r($prd);
            $fdata=array(
                ...
                $result['payment_firstname'].' ('.$result['telephone'].')',
                $this->model_localisation_order_status->getOrderStatus($result['order_status_id'])['name'],
                $result['total'],            
                $result['pay_method'],
                $billtype,
                $date,
        ...
        header('Content-Disposition: attachment;filename="product_storewisesales_'.date('dMy').'.csv"');
	...
	public function download_excel() 
        ...
        foreach ($results as $result)
        {
            ...
            $fdata=array(
                ...
                $billtype,
                $date,
                json_encode($result['order_product'])
                );
                fputcsv($fileIO,  $fdata,",");
