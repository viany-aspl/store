...
error_reporting(0);
class ControllerProcurementAccount extends Controller{
		...
		$results = $this->model_procurement_purchase_order->confirm_order_by_account($order_id);
		$this->model_purchase_procurement_order->add_po_trans($order_id,$from,'7',$user_id);
		...
		if (!empty($this->request->get['page'])) {
			$url .= '&page=' . $this->request->get['page'];
                ...
                $this->load->model('procurement/purchase_order');
		$results = $this->model_procurement_purchase_order->cancel_order_by_account($order_id,$reject_Message,$user_id);
		$this->model_procurement_purchase_order->add_po_trans($order_id,$from,'6',$user_id);
        ...
        { //echo $order_id.",".$suppliers_id;
            ...
            $body = "<p>Purchase request link"
                    . "<br/><br/>"
                    . "Akshamaala Solution Pvt. Ltd.</p>";
            $body.="<br/><br/>https://unnati.world/shop/index.php?route=supplier/supplier/procurement&order_id=".$order_id."&supplier_id=".$suppliers_id;
		...
		$mail->AddCC('vipin.kumar@aspltech.com', "Vipin");
                	$mail->AddCC('subhash.jha@unnati.world', "Subhash");
	...
	public function receive_order()
	{
		...
		$this->load->model('procurement/purchase_order');
		$data['order_information'] = $this->model_procurement_purchase_order->view_order_details_by_account($order_id);
		...
		$data['action'] = $this->url->link('procurement/account/insert_receive_order', 'token=' . $this->session->data['token'] . $url, true);
		...
		$this->response->setOutput($this->load->view('procurement/receive_order.tpl',$data));
	...
	public function insert_receive_order()
	{
		...
		{ 
			...
			$this->load->model('purchase/supplier');
			$data['suppliers'] = $this->model_purchase_supplier->get_total_suppliers();
			$this->response->redirect($this->url->link('procurement/account/receive_order', 'token=' . $this->session->data['token'] . $url, true));
			$this->response->setOutput($this->load->view('procurement/receive_order.tpl',$data));
		}
		else
		{
			...
			$this->load->model('procurement/purchase_order');
                        ...
                        $this->model_procurement_purchase_order->add_po_trans($order_id,'5','7',$user_id);
			$inserted = $this->model_procurement_purchase_order->insert_receive_order($received_order_info,$order_id);
			if($inserted)
			{
			...
			else
			{
				...
				$this->response->redirect($this->url->link('procurement/account/insert_receive_order', 'token=' . $this->session->data['token'] . $url, true));
			}
