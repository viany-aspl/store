...
class ControllerMarketingCoupon extends Controller {
	...
	protected function getForm() {
		...
		$data['error_warning'] = '';
		$this->error['coupon_store'];
		if (isset($this->error['warning'])) {
			$data['error_warning'] = $this->error['warning'];
		} else {
			$data['error_warning'] = '';
		}
		if (isset($this->error['coupon_store'])) {
			$data['error_warning'] = 'Please select at least one store';
		} else {
			//$data['error_warning'] = '';
		}
		if (isset($this->error['coupon_product'])) {
			$data['error_warning'] = 'Please Select at least one product OR Category';
		} else {
			//$data['error_warning'] = '';
		}
		...
		///////////////////
		if (isset($this->request->post['coupon_store'])) {
			$stores = $this->request->post['coupon_store'];
		} elseif (isset($this->request->get['coupon_id'])) {
			$stores = $this->model_marketing_coupon->getCouponStores($this->request->get['coupon_id']);
		} else {
			$stores = array();
		}
	...
	protected function validateForm() {
		...
		if (count($this->request->post['coupon_store']) < 1)  {
			$this->error['coupon_store'] = $this->language->get('coupon_store');
		}
		if ((count($this->request->post['coupon_product']) < 1) && (count($this->request->post['coupon_category']) < 1))  {
			$this->error['coupon_product'] = 'Please Select at least one product OR Category';
		}
	...
	public function getAllStores()
	{
		...
		$data['coupon_store'] = array();
		$stores = $this->model_marketing_coupon->getAllStores();
		//print_r($stores);
		$return_data="";
		foreach ($stores as $store) {
			//print_r($store);
			$return_data.='<div id="coupon-store'.$store['store_id'].'"><i class="fa fa-minus-circle"></i> '.$store['name'].'<input type="hidden" name="coupon_store[]" value="'.$store['store_id'].'" /></div>';
		}
		echo $return_data;
	}
}
