...
class ControllerInventoryPurchaseOrder extends Controller{
	public function index()
	{
		//set the title of the page
			...
			'href' => $this->url->link('inventory/purchase_order', 'token=' . $this->session->data['token'] . $url, true)
		);

		$data['add'] = $this->url->link('inventory/purchase_order/add', 'token=' . $this->session->data['token'] . $url, true);
		$data['delete'] = $this->url->link('inventory/purchase_order/delete', 'token=' . $this->session->data['token'] . $url, true);
		$data['filter'] = $this->url->link('inventory/purchase_order/filter', 'token=' . $this->session->data['token'] . $url, true);
		...
		$data['order_list'] = $this->model_inventory_purchase_order->getList($start,$limit);
		/*getting the list of the orders*/
		...
		//getting total orders
		$data['view'] = $this->url->link('inventory/purchase_order/view_order_details', 'token=' . $this->session->data['token'] . $url, true);
		$data['receive'] = $this->url->link('inventory/purchase_order/receive_order', 'token=' . $this->session->data['token'] . $url, true);
		//getting pages
		...
		$pagination->url = $this->url->link('inventory/purchase_order', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('inventory/purchase_order_list.tpl', $data));
	}
	...
	public function add()
	{
		...
		$url = '';
			...
			$data['action'] = $this->url->link('inventory/purchase_order/insert_purchase_order', 'token=' . $this->session->data['token'] . $url, true);
			$data['cancel'] = $this->url->link('inventory/purchase_order', 'token=' . $this->session->data['token'] . $url, true);
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('text_home'),
			'href' => $this->url->link('common/dashboard', 'token=' . $this->session->data['token'], true)
		);
		...
		/*getting the product options from database*/
		//$this->load->model('inventory/supplier');
		//$data['suppliers'] = $this->model_inventory_supplier->get_total_suppliers();
...
//stores
            ...
            if(isset($user['user_group_id']) && $user['user_group_id']== $pos_user_group_id )
	{
		...
		$this->response->setOutput($this->load->view('inventory/purchase_order_form.tpl', $data));
	}
	...
	public function delete()
	{
		...
		$this->load->model('inventory/purchase_order');
		$deleted = $this->model_inventory_purchase_order->delete($ids);
		if($deleted)
		{
			$_SESSION['delete_success_message'] = "The order successfully deleted";
			$this->response->redirect($this->url->link('inventory/purchase_order', 'token=' . $this->session->data['token'] . $url, true));
		}
		else
		{
	...
	public function insert_purchase_order()
	{
		...
		else
		{
			...
			if(isset($this->request->post['mail_bit']))
			{
				...
				$html = $this->load->view('inventory/mail_purchase_order.tpl',$data);
	...
	public function view_order_details()
	{
		...
		$data['order_information'] = $this->model_inventory_purchase_order->view_order_details($order_id);
		$data['cancel'] = $this->url->link('inventory/purchase_order', 'token=' . $this->session->data['token'] . $url, true);
		$data['pdf_export'] = $this->url->link('inventory/purchase_order/view_order_details', 'token=' . $this->session->data['token'] . $url, true);
		if(isset($_GET['export']))
		{
			...
			$html = $this->load->view('inventory/print_order.tpl',$data);
		...
		{
			$this->response->setOutput($this->load->view('inventory/view_order.tpl',$data));
		}
	}
...
$log->write("filter call 1");
		...
		if(count(array_filter($post,'strlen')) != 0)
		{
			...
			$data['order_list'] = $this->model_inventory_purchase_order->filter($filter,$start,$limit);
			if(!$data['order_list'])
			{
			...
			else
			{
				...
				$total_orders = $this->model_inventory_purchase_order->filterCount($filter);
			}
		}
		else
		{
		...
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('inventory/purchase_order/filter', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
	...
	public function receive_order()
	{
		...
		$data['action'] = $this->url->link('inventory/purchase_order/insert_receive_order', 'token=' . $this->session->data['token'] . $url, true);
		$data['cancel'] = $this->url->link('inventory/purchase_order', 'token=' . $this->session->data['token'] . $url, true);
		//$this->load->model('inventory/supplier');
		//$data['suppliers'] = $this->model_purchase_supplier->get_total_suppliers();
		$this->response->setOutput($this->load->view('inventory/receive_order.tpl',$data));
...
$log->write("before check");
		if((count($received_quantities) != count(array_filter($received_quantities))) || (count($prices) != count(array_filter($prices))) || $order_receive_date == '')
		{
			...
			//$this->load->model('inventory/supplier');
			//$data['suppliers'] = $this->model_inventory_supplier->get_total_suppliers();
			//$this->response->redirect($this->url->link('inventory/purchase_order/receive_order', 'token=' . $this->session->data['token'] . $url, true));
			$this->response->setOutput($this->load->view('inventory/receive_order.tpl',$data));
		}
		else
		{
$log->write("after check");
			...
			else
			{
				...
				$this->response->redirect($this->url->link('inventory/purchase_order/insert_receive_order', 'token=' . $this->session->data['token'] . $url, true));
			}
