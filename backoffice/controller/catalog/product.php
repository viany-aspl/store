...
//exit;
	...
	public function promotion() 
	{
		...
		$this->document->setTitle('Product Promotion');
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST')  
		{
			...
			$this->model_catalog_product->editProductpromotion($this->request->get['product_id'], $this->request->post);
	...
	protected function getpromotionForm() 
	{
		$data['heading_title'] = 'Product Promotion';
		
		$data['text_form'] = !isset($this->request->get['product_id']) ? $this->language->get('text_add') : $this->language->get('text_edit');
		$data['text_enabled'] = $this->language->get('text_enabled');
		$data['text_disabled'] = $this->language->get('text_disabled');
		
		if (isset($this->error['hstn'])) {
			$data['error_hstn'] = $this->error['hstn'];
		} else {
			$data['error_hstn'] = '';
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/product/promotion', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		$data['action'] = $this->url->link('catalog/product/promotion', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, 'SSL');
		...
		if((!empty($product_info['promotion_start_date'])) && (!empty($product_info['promotion_end_date'])))
		{
			$data['promotion_start_date'] =date('Y-m-d',$product_info['promotion_start_date']->sec);
			$data['promotion_end_date'] =date('Y-m-d',$product_info['promotion_end_date']->sec);
		...
		$this->response->setOutput($this->load->view('catalog/product_promotion_form.tpl', $data));
	}
	...
	protected function getList() 
	{
		...
		foreach ($results as $result) 
        {
			...
			{
				$ProductImage1=$this->model_catalog_product->getProductImage($result['product_id']);
				//print_r($ProductImage1);
				$result['image']=$ProductImage1->row['image'];
			}
			...
			foreach ($product_specials  as $product_special) 
				...
				'edit_promotion'=>$this->url->link('catalog/product/promotion', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url, 'SSL'),
                            );
	...
	protected function getForm() 
	{
		...
		if (isset($this->request->post['hstn'])) {
			$data['hstn'] = $this->request->post['hstn'];
		} elseif (!empty($product_info)) {
			$data['hstn'] = $product_info['HSTN'];
		} else {
			$data['hstn'] = '';
		}
		...
		if (isset($this->request->post['product_recurrings'])) {
			...
			$data['product_recurrings'] = array();//$this->model_catalog_product->getRecurrings($product_info['product_id']);
		} else {
		...
		if (isset($this->request->post['tax_class_id'])) {
		    ...
		    $data['price_tax'] = '';
                	...
                	$data["price_tax"]=$product_info["price_tax"];
		...
		foreach ($products as $product_id) {
			$related_info = $this->model_catalog_product->getCrop($product_id);
                        
			if ($related_info) {
				$data['product_relateds'][] = array(
					'product_id' => $related_info['sid'],
		...
		$data['hsn_codes'] = $this->model_catalog_product->getHsnCodes(); 
	...
	protected function validateForm() {
		...
		if ((utf8_strlen($this->request->post['hstn']) < 1) || (utf8_strlen($this->request->post['hstn']) > 64)) {
			$this->error['hstn'] = 'Please enter HSN Code';
		} 
	...
	public function autocomplete_unverified() {
		$json = array();
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model'])) {
			...
			$results = $this->model_catalog_product->getProductsUnverified($filter_data)->rows;
...
public function autocomplete_crops() {
	...
	public function download_excel()
	{
		...
		$this->load->model('catalog/product');
		$results = $this->model_catalog_product->getProductsWithTaxAndCategory($filter_data);
                //print_r($results);
		...
		foreach ($results as $result) 
		{
                    $sqarray=$this->model_catalog_product->getProductStoresQuantity($result['product_id']);
			
                        $squantity=0;
			foreach($sqarray as $sqarray2)
			{
				$squantity=$squantity+$sqarray2['quantity'];
			}
                        $fdata=array(
                            $result['product_id'],
                            $result['name'],
                            ...
                            $result['price_tax'],
                            $squantity,
                            $result['tax_class_name'],
                            implode("-",$result['category_name']),
                            ($result['status']) ? $this->language->get('text_enabled') : $this->language->get('text_disabled'),
							$result['user_category_id'],
                            $result['user_category_name']
                            );
    		...
    		header('Content-Disposition: attachment;filename="Products_list_'.date('dMy').'.csv"');
