...
class ControllerSettingStore extends Controller 
{
	...
	$results = $this->model_setting_store->getStoresWeb($filter_data);
        ...
        $store_types=$this->model_setting_store->getstoretypes();
       ...
       foreach ($results as $result)
        {
		   ...
		   $user_info = $this->model_setting_store->getStoreUser($result["store_id"]);
		   //print_r($store_info);exit;
			foreach($store_types as $store_type)
			{
				if($store_type['sid']==$store_info['config_storetype'])
				{
					$config_storetype = $store_type['type_name'];
				}
			}
           ...
           $fdata=array(
                            ...
                            $store_info['config_name'],
                            $result["store_id"],
                            $config_storetype,
                            $store_info['config_owner'],
							$store_info['config_telephone'],
                           $store_info['config_gstn'],
						   $store_info['config_geocode'],
          ...
          fseek($fileIO, 0);
            ...
            header('Content-Disposition: attachment;filename="store_list'.date('dMy').'.csv"');
	...
	protected function getList() 
	{
		...
		$data['add'] = $this->url->link('setting/setting', 'token=' . $this->session->data['token'], 'SSL');
		...
		{ //print_r($result);
			$data['stores'][] = array(
				...
				'edit'     => $this->url->link('setting/setting', 'token=' . $this->session->data['token'] . '&store_id=' . $result['store_id'], 'SSL'),
                                'view'     => $this->url->link('setting/setting/view', 'token=' . $this->session->data['token'] . '&store_id=' . $result['store_id'], 'SSL'),
                                'document'     => $this->url->link('setting/setting/document', 'token=' . $this->session->data['token'] . '&store_id=' . $result['store_id'], 'SSL')
			);
		}
	...
	public function getForm() 
	{
		...
		if (isset($this->error['storetype'])) {
                 		$data['error_storetype'] = 'Please Select Store Type';
               	} else {
		...
		if (isset($this->request->post['creditlimit'])) {
                		$data['creditlimit'] = $this->request->post['creditlimit'];
                	}
                	elseif (isset($store_info['creditlimit'])) {
                		 $data['creditlimit'] = $store_info['creditlimit'];
                	}
                	else {
                 		$data['creditlimit'] = $this->config->get('creditlimit');
                	}
                //print_r($store_info);

               	 if (isset($this->request->post['currentcredit'])) {
                		$data['currentcredit'] = $this->request->post['currentcredit'];
                	}
               	elseif (isset($store_info['currentcredit'])) {
                 		$data['currentcredit'] = $store_info['currentcredit'];
                	}
                	else {
                 		$data['currentcredit'] = $this->config->get('currentcredit');
                	}
	...
	protected function validateForm() 
	{
		...
		if (!$this->request->post['creditlimit']) {
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_store']) || isset($this->request->get['filter_model'])) {
			$this->load->model('setting/store');
