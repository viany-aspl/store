<?php
	...
	class ControllerPartnerPendingOrders extends Controller {
		public function index() {
			...
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('partner/pending_orders', 'token=' . $this->session->data['token'] . $url, true)
			);
			
			
			$this->load->model('partner/pending_orders');
			$data['pending_orders'] = $this->model_partner_pending_orders->get_all_pending_orders();
			
			$data['pdf_export'] = $this->url->link('partner/pending_orders', 'token=' . $this->session->data['token'] . $url, true);
			...
			if(isset($this->request->get['export']) && isset($this->request->get['page_no']))
			{
				...
				$html = $this->load->view('partner/print_pending_orders.tpl',$data);
			...
			else
			{
				...
				$pagination->url = $this->url->link('partner/pending_orders', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
				...
				$this->response->setOutput($this->load->view('partner/pending_orders.tpl', $data));
			}
		}
	}
?>