...
error_reporting(0);
class ControllerPurchaseCm extends Controller{
		...
		$results = $this->model_purchase_purchase_order->confirm_order_by_cm($order_id);
		$this->model_purchase_purchase_order->add_po_trans($order_id,$from,'5',$user_id);
		...
		if (!empty($this->request->get['page'])) {
			$url .= '&page=' . $this->request->get['page'];
                ...
                $from=$this->request->get['order_status_id'];
                $this->load->model('purchase/purchase_order');
		$results = $this->model_purchase_purchase_order->cancel_order_by_cm($order_id,$reject_Message,$user_id);
		$this->model_purchase_purchase_order->add_po_trans($order_id,$from,'4',$user_id);
