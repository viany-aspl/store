...
error_reporting(0);
class ControllerPurchaseRm extends Controller{
		...
		$results = $this->model_purchase_purchase_order->confirm_order_by_rm($order_id);
                $this->model_purchase_purchase_order->add_po_trans($order_id,$from,'3',$user_id);
		...
		if (!empty($this->request->get['page'])) {
			$url .= '&page=' . $this->request->get['page'];
                ...
                $this->load->model('purchase/purchase_order');
		$results = $this->model_purchase_purchase_order->cancel_order_by_rm($order_id,$reject_Message,$user_id);
                $this->model_purchase_purchase_order->add_po_trans($order_id,$from,'2',$user_id);
