...
$match=array(
                "based"=>'store',
                'tr2cg.customer_group_id'=>(int)$this->config->get('config_customer_group_id'),
                'z2gz.country_id'=>(int)$country_id,
                'z2gz.zone_id'=>(int)$zone_id 
                );
            $lookup='';
            ...
            $limit='';
            $start='';
            $log=new Log("tax-cls-".date('Y-m-d').".log");
	...
	public function getRateName($tax_rate_id) 
        {
            $tax_query = $this->db->query("select",DB_PREFIX . "tax_rate",'','','',array('tax_rate_id'=>(int)$tax_rate_id));    
