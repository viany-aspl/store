...
class socketWebSocket extends socket
{
	...
	private function run()
	{
		while(true)
		{
				...
				else
				{
					...
					else
					{
						...
						else
						{
							...
							{
								$name = md5(mt_rand());//substr($action,$pos+7);
								$this->users[$socket_index] = $name;
								$this->console('Loged in as: '.$name);
                                                                $this->console('Loged in socket: '.$socket);
                                                                $output = array();                                                                
                                                                $output['message'] =$this->mcrypt->encrypt(($name)).",".$this->mcrypt->encrypt(($socket_index));                                                                
                                                                $this->send($socket, json_encode($output));
				
							}
                                                        else if(($pos=strpos($action,'auth'))===0)
							{
                                                               $this->console($action);
                                                                $str=trim(str_replace("auth:","",$action));
                                                                $sdata= json_decode($str,true);
                                                                $name=$this->users[$sdata["id"]] ;
								$this->console('Loged in as: '.$name);
                                                                $output = array();   
                                                                ...
                                                                foreach($this->users as $user)
                                                                {
                                                                    if($sdata['token']==$user)
                                                                    {
                                                                        $this->console('Loged in done');                                                                                                                                   
                                                                        $output['token'] = $sdata['token'];
                                                                        $output['users'] = $sdata['user'].'-'.$sdata['store'];
                                                                        //$output['suser'] = $sdata['store'];
									$this->console(sizeof($this->allsockets));	
									//$skipSockets = array($this->master,$socket);
									//$new_sockets = array_diff($this->allsockets,$skipSockets);		
									//foreach( $new_sockets as $cli_socket ){
										$this->console('in loop \n');
									//$this->console($cli_socket.'\n');												
                                                                        $this->send($this->clients[$sdata['id']]->socket_id, json_encode($output));
									//}
                                                                        
                                                                        break;    
                                                                    }
                                                                    else {$output['message'] = "no" ;}
			...
			//server messages
			/*if($timeDiff>10000)//send messages out every 1000 ms
			{
				$output = array();
				$this->lastTime = microtime(true);
				$output['ping'] = "Server Message";
