                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title pull-left">Update Quantity (<span id='p_name'></span>)</h5>
                                        </div>
                                        <div class="modal-body">
                                            <div class="form-group">
                                            
                                                <input type="number" min="1" step="1" class="form-control numberinput" placeholder="Add Quantity" name="new_quantity" id="new_quantity" />
                                        ...
                                        <div class="modal-footer">
                                            <button type="button" class="btn btn-link" onclick="return update_price();">Add Quantity</button>
...
function update_price()
{
    ...
    if(new_quantity)
    {
        ...
        var cnfrm=confirm('Are you sure ? You want to add quantity for '+p_name);
        if(cnfrm)
        {
            $.ajax({
			url: 'index.php?route=pos/inventory_manager/update_quantity&token=<?php echo $token; ?>&product_id=' +prd_id+'&new_quantity=' +  encodeURIComponent(new_quantity),
			dataType: 'json',
			beforeSend: function() 
			{
			...
			success: function(json) 
				...
				$("#please_wait_span").remove();
				$(".page-loader").hide();
                            alertnotify('Quantity successfully added');
...
try{
    ...
    $.post('index.php?route=pos/inventory_manager/getCategoryItems&token=<?php echo $token; ?>',{ category_id: $id, page: $page }, function(data)
    {
        ...
        for(var i = 0; i < data.products.length; i++)
        {
            ...
            html += '<button class="btn btn-secondary" id="quantity'+data.products[i]['id']+'">'+data.products[i]['stock_text']+'</button>';
            html += '</div>';
...
{ 
	...
	'select': function(item) 
		...
		$('input[name=\'filter_product\']').val(item['label']);
                    ...
                    html += '<div class="row">';
                    html += '<div class="col-sm-6">';
                    html += '<button class="btn btn-secondary" id="quantity'+item['value']+'">'+item['stock_text']+'</button>';
                    html += '</div>';
                    html += '<div class="col-sm-6">';
